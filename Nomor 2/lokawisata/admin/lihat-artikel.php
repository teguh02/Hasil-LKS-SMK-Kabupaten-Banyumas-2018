						...
						<div class="t-content t-padding-2 t-padding-top-1">
							...
							<?php
								if (isset($_GET['id'])) {
									...
									$sql1 = "SELECT * FROM belum WHERE id='$id'";
							...
							<form action="setujui.php" method="post" class="t-margin-top-1">
								...
								<input class="t-input t-margin-top-1 t-large" value="<?php echo $ambil['judul'] ?>" type="text" name="judul" placeholder="Masukan Kata Sandi">

								<textarea class="t-input t-margin-top-1 isi t-large" name="isi" ><?php echo $ambil['isi'] ?></textarea>

								<center>
									<input type="submit" name="setuju" class="t-input kirim t-margin-top-1 t-blue" value="Setujui Artikel">
									<?php
										echo "<a class='hapus t-input t-blue t-margin-top-1' href='hapus-artikel.php?id=".$ambil['id']."'>Hapus Artikel</a>";
									?>
								</center>

								<center><a class="batal" href="artikel.php">Batal</a></center>
							</form>
						</div>
					</div>
				</div>
